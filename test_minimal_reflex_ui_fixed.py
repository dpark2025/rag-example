#!/usr/bin/env python3
"""Fixed comprehensive test for the minimal Reflex UI using Playwright."""
...
async def test_reflex_ui():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Send a test message
            test_message = "Test message for UI verification"
            await chat_input.fill(test_message)
            await send_button.click()
            
            # Wait for chat area to change (messages should appear)
            await page.wait_for_timeout(2000)  # Give time for state update
            
            # Check that the message area has changed from welcome to messages
            messages_area = page.locator('div').filter(has_text="Test message for UI verification").first()
            await expect(messages_area).to_be_visible()
            print("✅ User message appears in chat")
            
            # Check for AI response (using partial text match)
            ai_response = page.locator('text=I understand you asked:').first()
